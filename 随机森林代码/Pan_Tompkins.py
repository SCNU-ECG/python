...
def squaring(signal):
    ...
    for index in range(len(signal)):
      result[index] = signal[index]**2
    return result    
def move_window_integration(signal):
    '''
...
def solve(signal):
    ...
    # Bandpass Filter
    global bpass
    bpass = band_pass_filter(input_signal.copy())
    # Derivative Function
    global der
    ...
    global mwin
    mwin = move_window_integration(sqr.copy())
    return mwin
...
class heart_rate():
  ...
  def __init__(self,signal,samp_freq, mwin):
    '''
